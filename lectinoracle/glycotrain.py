...
from extract import create_parser, main as extract_main
...
def read_data(pickle_data, **kwargs):
    ...
    for sample in all_data["data"]:
        ...
        data[sample["split"]].append((lec, gly, sample["label"]))

    loaders = []
    for split in ("train", "val", "test"):
        df = pd.DataFrame(data[split], columns=['seq', 'glycan', 'match'])
        x = list(zip(df.seq.values.tolist(), [glycan_to_graph(k, libr=kwargs["gly_lib"])
                                              for k in df.glycan.values.tolist()]))
        ...
        loaders.append(DataLoader(dataset, batch_size=128, shuffle=True))
...
def train_lo(**kwargs):
    ...
    start = time.time()
    train_loader, val_loader, _ = read_data(kwargs["data"], **kwargs)
    ...
    # save model
    torch.save(model.state_dict(), os.path.join("checkpoints", "lo", "regression",
                                                f"oracle_{kwargs['data'].split('/')[-1].split('_')[0]}.pth"))
